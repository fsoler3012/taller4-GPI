...
# 1) Define la ruta real de tu raíz correcta
root <- "C:/Users/FSolerMa/OneDrive - JNJ/UniAndes/GPI/taller4-GPI"

# 2) Crea el archivo .here en esa raíz (si no existe)
if (!file.exists(file.path(root, ".here"))) {
  file.create(file.path(root, ".here"))
}

# 3) (Opcional) Muestra confirmación
file.exists(file.path(root, ".here"))
